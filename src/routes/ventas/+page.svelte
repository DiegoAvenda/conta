...
<div class="min-h-screen bg-[#f7f7f5] font-sans text-[#171717]">
	<header
		class="flex h-18 items-center justify-between border-b border-[#e5e5e5] bg-white px-8 max-[600px]:px-4.5"
	>
		...
		<div class="flex flex-col items-end gap-0.75 text-xs text-[#737373]">
			<span>Periodo</span>
	...
	>
		{#if activeView === 'home'}
			<section class="mb-10">
				<div class="mb-3.5 text-[11px] font-extrabold tracking-[0.14em] text-[#737373]">
					VENTAS DIRECTAS
				</div>
				...
				<p class="mt-4.5 max-w-145 text-base leading-[1.6] text-[#666]">
			...
			<section class="flex flex-col gap-3">
				<button
					class="flex w-full items-center gap-4.5 rounded-[15px] border border-[#dedede] bg-white p-5.5 text-left transition duration-150 hover:-translate-y-px hover:border-[#999] max-[600px]:p-4.5"
					onclick={() => (activeView = 'sales')}
				>
					<div
						class="flex h-10.5 w-10.5 shrink-0 basis-10.5 items-center justify-center rounded-[10px] bg-[#f1f1ef] text-[21px] max-[600px]:h-9 max-[600px]:w-9 max-[600px]:basis-9"
					>
						≡
					</div>
			...
			<section class="mt-7 rounded-xl bg-[#efefed] px-5 py-4.5">
				<strong class="text-[13px]">💡 Todo queda registrado de forma automática.</strong>

				<p class="mt-1.5 text-[13px] leading-[1.5] text-[#666]">
		...
		{:else}
			...
			<section class="rounded-2xl border border-[#dedede] bg-white p-7 max-[600px]:p-5">
				...
				<div class="overflow-x-auto">
					<div
						class="grid min-w-145 grid-cols-[1fr_1.2fr_1fr_1fr_30px] items-center gap-3 border-b border-[#eee] py-[14px] pt-0 text-[11px] font-bold text-[#888]"
					>
						<span>Fecha</span>
					...
					{#each data.ventas as venta (venta._id)}
						<div
							class="grid min-w-145 grid-cols-[1fr_1.2fr_1fr_1fr_30px] items-center gap-3 border-b border-[#eee] py-[14px] text-[13px]"
						>
							<span>
			...
			<section
				class="mt-4.5 flex items-center justify-between gap-5 rounded-[14px] bg-[#171717] p-5 text-white max-[600px]:flex-col max-[600px]:items-start"
			>
				<div>
					<strong class="text-sm">¿Terminaste de registrar tus ventas?</strong>

					<p class="mt-1.25 max-w-112.5 text-xs leading-normal text-[#bbb]">
						Contaco combinará estas ventas directas con tus gastos para preparar tu cierre mensual.
					</p>
				</div>

				<button
					class="flex-[0_0_auto] rounded-lg border-0 bg-white px-3.75 py-2.75 text-xs font-bold text-[#171717] max-[600px]:w-full"
				>
					Continuar →
				</button>
			</section>
